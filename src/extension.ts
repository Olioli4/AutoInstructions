...
// Remove the local variable and always read from configuration
function getStandardInstructions(): string {
	const config = vscode.workspace.getConfiguration('auto-instructions');
	return config.get<string>('standardInstructions', DEFAULT_INSTRUCTIONS);
}
...
function ensureInstructions(workspaceFolder: string): string {
	...
	const standardInstructions = getStandardInstructions();
...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('auto-instructions.changeStandardInstructions', async () => {
		const config = vscode.workspace.getConfiguration('auto-instructions');
		const current = config.get<string>('standardInstructions', DEFAULT_INSTRUCTIONS);
		...
		if (newInstructions !== undefined) {
			await config.update('standardInstructions', newInstructions, vscode.ConfigurationTarget.Global);
			vscode.window.showInformationMessage('Standard instructions updated in settings. New workspaces will use this content.');
		}
	});
